...
export default function RoomModal({ room, onClose, waNumber = "6281234567890", kosName = "Garuda Kostel" }) {
  ...
  // Reset slider ke gambar pertama setiap kali modal baru dibuka dengan kamar berbeda
  useEffect(() => {
    if (room) setCurrentIndex(0);
  }, [room]);

  // Efek ganti gambar otomatis setiap 4 detik (sedikit lebih santai dari 3 detik)
  useEffect(() => {
    let timer;
    ...
    return () => clearInterval(timer); // Bersihkan timer saat ditutup
  }, [room, images.length]);
  ...
  // Modifikasi teks WhatsApp menggunakan prop kosName agar dinamis
  const waMessage = room ? `Halo Admin ${kosName}, saya tertarik dengan Kamar ${room.number} (Lantai ${room.floor}). Apakah masih tersedia?` : '';
          ...
          >
            ...
            <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-white/80 dark:bg-gray-800/80 backdrop-blur-md z-20">
              <div>
                ...
                <p className="text-xs font-bold text-gray-500 dark:text-gray-400 mt-1">Lantai {room.floor}</p>
              </div>
            ...
            <div className="overflow-y-auto p-6 space-y-6 hide-scrollbar">
                ...
                <div className="relative w-full h-56 md:h-72 rounded-[2rem] overflow-hidden bg-gray-100 dark:bg-gray-900 shadow-inner group border border-gray-100 dark:border-gray-700">
                  <AnimatePresence initial={false} custom={currentIndex}>
                    <motion.img
                      ...
                      alt={`Kamar ${room.number} - Foto ${currentIndex + 1}`}
              ...
              {/* Rincian Harga & Status */}
              <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-3xl border border-blue-100 dark:border-blue-800/50">
                <div className="flex justify-between items-start mb-4">
                  <span className="text-xs font-black uppercase tracking-widest text-blue-500 dark:text-blue-400">Status Kamar</span>
                  <span className={`px-3 py-1 text-[10px] font-black rounded-full uppercase shadow-sm ${
                    ...
                    {room.status === 'Available' ? 'Tersedia' : room.status === 'Booked' ? 'Penuh' : 'Perbaikan'}
                  </span>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-bold mb-1">Bulanan</p>
                    <p className="text-xl md:text-2xl font-black text-gray-900 dark:text-white">{formatRupiah(room.priceMonthly)}</p>
                  </div>
                  {room.priceDaily && (
                    <div>
                      <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-bold mb-1">Harian</p>
            ...
            <div className="p-5 md:p-6 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex gap-3">
              {room.status === 'Available' ? (
                ...
                <button disabled className="flex-1 bg-gray-200 dark:bg-gray-700 text-gray-400 dark:text-gray-500 font-black py-4 rounded-2xl cursor-not-allowed">
